...
class L3NRendering447{
  ...
  reset(){this.started=now();this.pass=0;this.history=[];this.classes={shadow:0,gi:0,reflection:0,refraction:0,sample:0,terminate:0};this.last=null;this.recommendations=[];}
  observe(input={}){const variance=Math.max(0,Number(input.variance??input.noise??0)||0),luma=lum(input.hdrColor??input.color??[0,0,0]),rays=Math.max(0,Number(input.rays??input.totalRays??0)||0),ms=Math.max(0,Number(input.elapsedMs??0)||0),sample=Math.max(0,Number(input.sample??input.currentSamples??0)||0),noiseTarget=Math.max(0,Number(input.noiseTarget??0)||0);
    const entry={pass:++this.pass,time:Date.now(),variance,luma,rays,elapsedMs:ms,sample,noiseTarget};this.history.push(entry);if(this.history.length>128)this.history.shift();this.last=entry;this._learn();return this.snapshot();}
  ...
  _learn(){const h=this.history;if(h.length<2){this.recommendations=[];return;}const a=h[h.length-2],b=h[h.length-1],dv=b.variance-a.variance,dr=b.rays-a.rays,dt=Math.max(1,b.elapsedMs-a.elapsedMs),rps=dr>0?dr/(dt/1000):0;const rec=[];
    if(b.variance>.10&&dv>=-.002)rec.push('variance plateau: redirect budget to the responsible ray class instead of global SPP');
  ...
  snapshot(){const h=this.history,n=h.length,last=this.last;let varianceTrend=0;if(n>1)varianceTrend=h[n-1].variance-h[n-2].variance;return Object.freeze({version:VERSION,provider:'L3N Rendering variance+luma feedback',passes:n,last,varianceTrend,decisions:{...this.classes},recommendations:[...this.recommendations],elapsedMs:now()-this.started});}
  text(){const s=this.snapshot(),l=s.last||{};return [`L3N RENDERING v${VERSION}`,`passes: ${s.passes}`,`variance: ${Number(l.variance||0).toFixed(6)}`,`luma: ${Number(l.luma||0).toFixed(6)}`,`varianceTrend: ${Number(s.varianceTrend||0).toFixed(6)}`,`decisions: ${JSON.stringify(s.decisions)}`,`recommendations: ${s.recommendations.length?s.recommendations.join(' | '):'none'}`].join('\n');}
}
