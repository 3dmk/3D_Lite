...
if 'LitePixCore2Worker418.prewarm(job?.renderScene)' not in s:
    sig='var PathGIRenderer=Object.freeze({\n  async render(job,acceleration,onProgress=()=>{}){'
    if s.count(sig)!=1: raise SystemExit(f'release-blocking: exact PathGIRenderer signature count {s.count(sig)}')
