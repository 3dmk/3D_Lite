...
def patch(path: Path):
    ...
    text=replace_optional(text,"this.progressive=true;this.adaptive=true;this.minSamples=8;this.maxSamples=64;this.noiseThreshold=.03;\n    this.maxBounces=5;this.denoise='Off';","this.progressive=true;this.adaptive=true;this.minSamples=2;this.maxSamples=8;this.noiseThreshold=.035;\n    this.maxBounces=4;this.denoise='Quality';")
    ...
    text=replace_optional(text,'return this.noise(index)>settings.noiseThreshold;','const lowSampleThreshold=this.samples[index]<4?settings.noiseThreshold*.75:settings.noiseThreshold;\n    return this.noise(index)>lowSampleThreshold;')
    text=text.replace("legacyVersion:'3.99.9'",f"legacyVersion:'{VERSION}'").replace("legacyVersion:'3.99.10'",f"legacyVersion:'{VERSION}'")
    text=text.replace("const VERSION='3.99.9';",f"const VERSION='{VERSION}';").replace("const VERSION='3.99.10';",f"const VERSION='{VERSION}';")
    text=text.replace("window.ThreeDLiteVersion.version='3.99.9'",f"window.ThreeDLiteVersion.version='{VERSION}'").replace("window.ThreeDLiteVersion.version='3.99.10'",f"window.ThreeDLiteVersion.version='{VERSION}'")
    text=text.replace('// 3DLite Compiled Render Data v3.99.9',f'// 3DLite Compiled Render Data v{VERSION}').replace('// 3DLite Compiled Render Data v3.99.10',f'// 3DLite Compiled Render Data v{VERSION}')
    if text==original: raise RuntimeError('low-sample patch made no changes')
