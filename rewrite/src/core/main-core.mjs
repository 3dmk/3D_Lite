...
import {
  createAddModifierCommand,
  createRemoveModifierCommand,
  createUpdateModifierCommand,
  createMoveModifierCommand
} from './modifier-commands.mjs';
...
export class ThreeDLiteMainCore {
  ...
  constructor({ validator } = {}) {
    ...
    this.#state = Object.freeze({
      ...
      metadata: Object.freeze({ schema: 6 })
    });
  }
  ...
  updateGeometry(handle, updater) {
    ...
    for (const entry of this.entities.entries()) {
      if (sameHandle(entry.value.geometry, handle)) this.#markEntityEvaluationDirty(entry.handle);
    }
    this.generations.bumpMany(['geometry','topology','evaluation','renderScene']);
    this.evaluator.invalidate();
    return true;
  ...
  evaluateGeometry(handle, stack = []) {
    return this.evaluator.evaluate(handle, stack);
  }
  ...
  evaluateEntityGeometry(entityHandle) {
    ...
    return this.evaluator.evaluate(entity.geometry, entity.modifiers ?? [], {
      scope: key,
      revision: this.evaluationRevision(entityHandle)
    });
  }

  evaluationRevision(entityHandle) {
    return this.#evaluationRevisions.get(handleKey(entityHandle)) ?? 0;
  }
  ...
  updateEntity(handle, updater, dirtyDomains = ['scene','renderScene']) {
    ...
    const evaluationChanged = !sameNullableHandle(beforeGeometry, next.geometry)
      || beforeModifiers !== modifierStackSignature(next.modifiers);
    this.entities.update(handle, deepFreeze(next));

    const domains = new Set(dirtyDomains);
    if (evaluationChanged) {
      domains.add('evaluation');
      domains.add('renderScene');
  ...
  assignGeometry(entityHandle, geometryHandle = null) {
    ...
    }, ['scene','geometry','evaluation','renderScene']);
  }
  ...
  replaceModifierStack(entityHandle, stack = []) {
    ...
    return this.updateEntity(entityHandle, draft => {
      draft.modifiers = structuredClone(normalized);
    }, ['evaluation','renderScene']);
  }

  addModifier(entityHandle, modifier, index = null) {
    return this.execute(createAddModifierCommand(entityHandle, modifier, index));
  }

  removeModifier(entityHandle, modifierId) {
    return this.execute(createRemoveModifierCommand(entityHandle, modifierId));
  }

  updateModifier(entityHandle, modifierId, patch = {}) {
    return this.execute(createUpdateModifierCommand(entityHandle, modifierId, patch));
  }

  moveModifier(entityHandle, modifierId, toIndex) {
    return this.execute(createMoveModifierCommand(entityHandle, modifierId, toIndex));
  }
  ...
  renderSceneStamp() {
    return this.generations.stamp(['scene','geometry','topology','transform','material','evaluation','renderScene']);
  }
  ...
  #syncEntityEvaluationGraph(handle) {
    ...
    this.evaluationGraph.register(source, []);
    nodes.push(source);

    let previous = source;
    for (const modifier of entity.modifiers ?? []) {
      const node = `${prefix}:modifier:${modifier.id}`;
      this.evaluationGraph.register(node, [previous]);
      nodes.push(node);
      previous = node;
    }

    const renderNode = `${prefix}:render`;
    this.evaluationGraph.register(renderNode, [previous]);
...
function sameNullableHandle(a, b) {
  if (!a && !b) return true;
